...
vi.mock('../styles.ts', () => ({
  createLayersForRegion: vi.fn((regionId: string) => [
    {
      id: `geology-fill-${regionId}`,
      type: 'fill',
      source: `geology-${regionId}`,
      'source-layer': 'geology',
      paint: {},
    },
    {
      id: `geology-line-${regionId}`,
      type: 'line',
      source: `geology-${regionId}`,
      'source-layer': 'geology',
      paint: {},
...
function makeMap() {
  ...
  return {
    addSource: vi.fn((id: string, spec: object) => {
      sources.set(id, spec)
    }),
...
describe('region-manager', () => {
  ...
  describe('ensureRegionInitialized', () => {
    it('Cas 3 — normandie non initialisée: ajoute la source', async () => {
      const { initRegions, ensureRegionInitialized, _resetInitializedRegions } =
        await import('../region-manager.ts')
      _resetInitializedRegions()
      const map = makeMap()
  ...
  describe('loadRegion — france', () => {
    it('Cas 4 — depuis bretagne vers france: initialise les 12 manquantes', async () => {
      const { initRegions, loadRegion, _resetInitializedRegions } =
        await import('../region-manager.ts')
      _resetInitializedRegions()
      const map = makeMap()
      map.isSourceLoaded.mockReturnValue(true)
